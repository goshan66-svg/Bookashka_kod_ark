...
class Mega:
    ...
    def stream_audio_chunks(self, file_handle, file_key, is_public=False, file=None):
        """НАШ ОКОНЧАТЕЛЬНЫЙ МЕТОД: На лету качает, дешифрует и отдает чанки звука в память."""
        import requests
        
        from Crypto.Cipher import AES
        from Crypto.Util import Counter
        from mega.crypto import a32_to_str, base64_to_a32, decrypt_attr, base64_url_decode, str_to_a32, get_chunks
        
        # === ЖЕСТКАЯ КОНВЕРТАЦИЯ КЛЮЧА ИЗ ЛОГА: Убираем ошибку 'str' and 'str' ===
        if isinstance(file_key, str):
            # Если прилетела Base64 строка, переводим в числа a32
            file_key = base64_to_a32(file_key)
        elif isinstance(file_key, (tuple, list)):
            # Если прилетел массив/кортеж, проверяем, чтобы внутри были ЧИСЛА, а не строки
            file_key = [int(x) if isinstance(x, (str, int, float)) else x for x in file_key]
        
        if file is None:
            if is_public:
                file_key = base64_to_a32(file_key) if isinstance(file_key, str) else file_key
                file_data = self._api_request({'a': 'g', 'g': 1, 'p': file_handle})
            else:
                file_data = self._api_request({'a': 'g', 'g': 1, 'n': file_handle})

            # Оригинальные математические формулы расчета крипто-ключей MEGA
            k = (file_key[0] ^ file_key[4], file_key[1] ^ file_key[5],
                 file_key[2] ^ file_key[6], file_key[3] ^ file_key[7])
            iv = file_key[4:6] + (0, 0)
        else:
        ...
        # Запускаем потоковый requests к серверам MEGA
        input_file = requests.get(file_url, stream=True).raw
        ...
        # Читаем облако частями по оригинальной сетке чанков
        for chunk_start, chunk_size in get_chunks(file_size):
            chunk = input_file.read(chunk_size)
            if not chunk:
                break
                
            # Расшифровываем кусок звука прямо в оперативной памяти устройства
            clean_audio_chunk = aes.decrypt(chunk)
            
            # Выбрасываем чистый поток байт в прокси-сервер плеера!
            yield clean_audio_chunk
        ...
        with open(filename, 'rb') as input_file:
            ...
            if file_size > 0:
                for chunk_start, chunk_size in get_chunks(file_size):
                    ...
                    if hasattr(self, 'upload_progress'): self.upload_progress[dest_filename or os.path.basename(filename)] = upload_progress
